...
# return the result that we compute above
  ...
  names(metrics) <- c('total distance','max_deviation','aver_deviation','area')
  return(metrics)
}
# 3-(f)
# Extract datas from the three files
data1 <- read.csv('C:/data/train_trajectories.csv')
data2 <- read.csv('C:/data/train_measures.csv')
data3 <- read.csv('C:/data/test_trajectories.csv')
data1 <- as.matrix(data1)
data2 <- as.matrix(data2)
data3 <- as.matrix(data3)
# check my functions against the sample measures
n <- nrow(data2)
for (k in 1:n) {
  data0 <- data1[which(data1[,1]==data2[k,1] & data1[,2]==data2[k,2]),3:5]
  data0 <- trajectory_normalize(data0)
  data0_metrics <- c()
  data0_metrics <- metrics_calculate(data0)
  error <- c()
  for (j in 1:4) {
    error[j] <- data0_metrics[j]-data2[k,j+2]
  }
...
# There is no break, so the errors are all smaller than 0.01.
# My functions are right
# Then, compute the metrics above for the test trajectories
result = matrix(0,nrow = 5,ncol = 6)
for (k in 6:10) {
  data4 <- data3[which(data3[,1]==k),3:5]
  data4 <- trajectory_normalize(data4)
  data4_metrics <- metrics_calculate(data4)
  result[k-5,3:6] <- data4_metrics
}
result[,1] <- c(6,7,8,9,10)
result[,2] <- 1
colnames(result) <- c('subject_nr','count_trial','tot_dist','max_abs_dev','avg_abs_dev','AUC')
result <- as.data.frame(result)
result
# The 'result' is my testing result
